...
export const Seccion1PageNotificacionesCandidato = ({ notificaciones, fetchData, todasLeidas }) => {
  ...
  const obtenerIcono = (tipo) => {
    switch (tipo) {
      case 'postulacion':
        return <i className="fa-solid fa-file-signature icono-notificacion"></i>;
      case 'mensaje':
        return <i className="fa-solid fa-message icono-notificacion"></i>;
      case 'calificacion':
        return <i className="fa-solid fa-star icono-notificacion"></i>;
      case 'strike':
                ...
                <div className="d-flex align-items-center flex-grow-1">
                    ...
                    <div className="fila-info">
                        <div className='contenedor-descripcion'>
                            {['calificacion', 'strike'].includes(notificacion.Tipo_Evento) ? (
                                <span
                                    className={`texto-descripcion-link ${notificacion.Leida !== '1' ? 'noti-noleida' : ''}`}
                                    onClick={() => {
                                    navigate('/usuario-candidato/miperfil-candidato', {
                                        state: { abrirModalCalificaciones: notificacion.Tipo_Evento === 'calificacion' }
                                    });
                                    }}
                            ...
                            ) : notificacion.Tipo_Evento === 'postulacion' ? (
                                <span
                                    className={`texto-descripcion-link ${notificacion.Leida !== '1' ? 'noti-noleida' : ''}`}
                                    onClick={() => {
                                    navigate('/usuario-candidato/vacantes-candidato', {
                                        state: {
                                        seccionActiva: "detalles-vacante",
                                        vacanteID: notificacion.Vacante_ID
                                        }
                                    });
                                    }}
                                    style={{ cursor: 'pointer' }}
                            ...
                            ) : notificacion.Tipo_Evento === 'nueva_post' ?(
                                <span
                                  className={`texto-descripcion-link ${notificacion.Leida !== '1' ? 'noti-noleida' : ''}`}
                                  onClick={() => {
                                    navigate('/usuario-candidato/perfil-empresa', {
                                      state: {
                                        idEmpresa: notificacion.Perfil_Empresa,
