...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    SELECT 
        ...
        vacante.Titulo,
        vacante.Descripcion,
        COALESCE(modalidad_trabajo.Modalidad, 'Desconocido') AS Modalidad_Vacante,
        COALESCE(estado.Nombre, 'Desconocido') AS Estado_Vacante,
        vacante.Ubicacion,
        vacante.Fecha_Limite,
        vacante.Estatus,
        vacante.Fecha_Creacion
    FROM vacante
    LEFT JOIN modalidad_trabajo ON vacante.Modalidad = modalidad_trabajo.ID
    LEFT JOIN estado ON vacante.Estado = estado.ID
